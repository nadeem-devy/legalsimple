...
export default function ClientMessagesPage() {
  ...
  useEffect(() => {
    if (!selectedConversation?.lawyer_id) {
    ...
    const fetchCaseStatus = async () => {
      try {
        const res = await fetch(
          `/api/chat/case-status?other_user_id=${encodeURIComponent(selectedConversation.lawyer_id)}`
        );
        if (!res.ok) return;
        const data = await res.json();
        setCaseStatus({
    ...
    fetchCaseStatus();
  }, [selectedConversation?.lawyer_id]);
  ...
  const filteredConversations = conversations.filter((conv) =>
    conv.lawyer_name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  async function handleSend() {
    if ((!newMessage.trim() && pendingFiles.length === 0) || !selectedConversation || !currentUserId) return;
                  ...
                  filteredConversations.map((conversation) => (
                    ...
                    >
                      <Avatar className="h-10 w-10">
                        <AvatarFallback
                          className={`${getAvatarColor(conversation.lawyer_name)} text-white text-sm`}
                        >
                          {getInitials(conversation.lawyer_name)}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-slate-900 text-sm">
                            {conversation.lawyer_name}
                          </span>
                      ...
                      {conversation.unread_count > 0 && (
                        <Badge className="bg-emerald-500 text-white h-5 w-5 p-0 flex items-center justify-center text-xs">
            ...
            <>
              ...
              <CardHeader className="border-b py-3 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback
                      className={`${getAvatarColor(selectedConversation.lawyer_name)} text-white`}
                    >
                      {getInitials(selectedConversation.lawyer_name)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="font-semibold text-slate-900">
                      {selectedConversation.lawyer_name}
                    </h3>
                  ...
                  {caseStatus.status === "active" ? (
                    ...
                    <>
                      ...
                      <span className="text-xs opacity-75">
                        — Waiting for {selectedConversation.lawyer_name} to accept
                      </span>
                    </>
                  ) : caseStatus.status === "completed" ? (
                    <>
              ...
              <ScrollArea className="h-full p-4">
                {loadingMessages ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {isOtherTyping && (
                      <TypingIndicator
                        name={selectedConversation.lawyer_name}
                        avatarColor={getAvatarColor(selectedConversation.lawyer_name)}
                      />
                    )}
            ...
            <CardContent className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-slate-500">
                  Choose a lawyer to start messaging
                </p>
              </div>
            </CardContent>
          )}
