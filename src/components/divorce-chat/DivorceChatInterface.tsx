...
import { ChatMessage, QuestionType, QuestionOption } from "@/lib/divorce-chat/types";
import {
  ChatState,
  startChat,
  processAnswer,
  processCurrentQuestion,
} from "@/lib/divorce-chat/engine";
import { getQuestionById, DIVORCE_CHAT_QUESTIONS } from "@/lib/divorce-chat/questions";

interface DivorceChatInterfaceProps {
  caseId?: string;
...
export function DivorceChatInterface({
  caseId,
  onComplete,
}: DivorceChatInterfaceProps) {
  const router = useRouter();
  const storageKey = `legalsimple_divorce_chat_${caseId || 'draft'}`;
  ...
  // The form has many conditional branches, so we estimate ~40-50 questions for a typical path
  const estimatedTotalQuestions = 45;
  ...
  const stats = {
    ...
    bankAccounts: chatState.data.bankAccounts?.length || 0,
    retirement: chatState.data.retirementAccounts?.length || 0,
    debts: (chatState.data.communityDebts?.length || 0) + (chatState.data.separateDebts?.length || 0),
  };
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (currentQuestion.type === "info") {
      ...
      if (infoState.isComplete) {
        // Save inline (avoid stale closure)
        setIsSubmitting(true);
        setSaveFailed(false);
        try {
          const response = await fetch("/api/cases/save-intake", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ caseId, intakeType: "divorce_no_children_chat", data: infoState.data }),
          });
          ...
          setSavedCaseId(resolvedCaseId);
          toast.success("Questionnaire completed successfully!");
          if (onComplete) { onComplete(infoState.data); }
          else { router.push(`/cases/${resolvedCaseId}?generate=true`); }
        } catch (error) {
          console.error("Error saving intake:", error);
          setSaveFailed(true);
          toast.error("Failed to save your responses. Please try again.");
    ...
    if (
      (currentQuestion.id === "full_name" || currentQuestion.id === "spouse_full_name") &&
    ...
    // Check if complete — trigger save inline (not via separate callback to avoid stale closure)
  ...
  }, [chatState, currentQuestion, currentInput, selectedOptions, dateValue, caseId, onComplete, router, storageKey, historyKey]);
    ...
    return (
      <Popover>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-sm hover:shadow-md transition-all flex-shrink-0 mt-1">
        ...
        >
          <div className="bg-gradient-to-br from-emerald-600 to-teal-700 p-2.5">
  ...
  const renderInputField = () => {
    ...
    switch (currentQuestion.type) {
      case "info":
        return (
          <Button
            ...
            className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 h-12 text-base font-medium shadow-lg"
          >
            Continue
        ...
        return (
          <div className="flex gap-3">
            ...
            <Button
              ...
              className="h-12 px-6 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 rounded-xl"
            >
      ...
      case "stop":
        return (
          <div className="space-y-3">
            {/* Show redirect to Divorce With Children if stopped due to having children */}
            {currentQuestion.id === "children_redirect" && (
              <Button
          ...
          <div className="space-y-3">
            <div className="relative">
              <Textarea
                ...
                rows={4}
                spellCheck={true}
                className="resize-none text-base border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl"
              />
              <button
                type="button"
                ...
                className={cn(
                  ...
                  isRefining
                    ? "opacity-60 cursor-not-allowed border-emerald-200 bg-emerald-50 text-emerald-600"
                    : !currentInput.trim()
                      ? "opacity-40 cursor-not-allowed border-slate-200 bg-slate-50 text-slate-400"
                      : "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 hover:border-emerald-300 active:scale-[0.97]"
                )}
              >
                {isRefining ? (
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <Input
                ...
                onKeyPress={handleKeyPress}
                placeholder="0"
                className="pl-8 h-12 text-base border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl"
              />
            </div>
            <Button
              onClick={handleSubmit}
        ...
        return (
          <div className="flex gap-3">
            <Input
              ...
              maxLength={14}
              className={cn(
                "flex-1 h-12 text-base border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl",
        ...
        return (
          <div className="flex gap-3">
            <Input
              ...
              className="flex-1 h-12 text-base border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl"
            />
            <Button
              onClick={handleSubmit}
        ...
        return (
          <div className="flex gap-3">
            <Input
              ...
              placeholder="XXXX"
              maxLength={4}
              className="flex-1 h-12 text-base text-center tracking-[0.5em] font-mono border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl"
            />
            <Button
              onClick={handleSubmit}
      ...
      default: {
        const isNameField = ["full_name", "spouse_full_name", "maiden_name_input"].includes(currentQuestion.id);
        return (
          <div className="flex gap-3">
            <div className="relative flex-1">
              <Input
                ...
                spellCheck={true}
                className={cn(
                  "h-12 text-base border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl w-full",
                  !isNameField && "pr-[90px]"
                )}
              />
              {!isNameField && (
  ...
  const getAnswerDisplay = (message: ChatMessage) => {
    ...
    // For yes/no, show a friendly label
    if (question.type === "yesno") {
      return message.content.toLowerCase() === "yes" ? "Yes" : "No";
    }

    // For select, show the option label
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)] min-h-[600px]">
      ...
      <div className="px-6 py-4 bg-gradient-to-r from-slate-50 to-emerald-50 border-b border-slate-100">
        ...
        {/* Case Statistics */}
        {(stats.properties > 0 || stats.vehicles > 0 || stats.bankAccounts > 0 || stats.retirement > 0 || stats.debts > 0) && (
          <div className="flex flex-wrap gap-3 mt-4 animate-in fade-in-0 slide-in-from-left-4 duration-500">
            ...
            {stats.bankAccounts > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-full border border-slate-200 text-xs font-medium text-slate-600">
                <Building className="h-3.5 w-3.5 text-purple-500" />
                {stats.bankAccounts} {stats.bankAccounts === 1 ? "Bank Account" : "Bank Accounts"}
              </div>
            )}
            {stats.retirement > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-full border border-slate-200 text-xs font-medium text-slate-600">
            ...
            {stats.debts > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-full border border-slate-200 text-xs font-medium text-slate-600">
                ...
                {stats.debts} {stats.debts === 1 ? "Debt" : "Debts"}
              </div>
            )}
          </div>
        )}
        ...
        {chatState.messages.map((message, index) => (
          ...
          >
            {message.type === "assistant" && (
              <div className="flex-shrink-0 w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-md">
            ...
            <div className="flex items-start gap-2">
              ...
              {/* Show tooltip next to the current question */}
              {message.type === "assistant" &&
               message.id === chatState.messages[chatState.messages.length - 1]?.id &&
               currentQuestion?.tooltip && (
                renderTooltip()
