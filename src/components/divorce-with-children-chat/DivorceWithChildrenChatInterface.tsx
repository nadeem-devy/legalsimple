...
} from "@/lib/divorce-with-children-chat/engine";
import { getQuestionById, DIVORCE_WITH_CHILDREN_QUESTIONS } from "@/lib/divorce-with-children-chat/questions";

interface DivorceWithChildrenChatInterfaceProps {
  caseId?: string;
...
export function DivorceWithChildrenChatInterface({
  caseId,
  onComplete,
}: DivorceWithChildrenChatInterfaceProps) {
  const router = useRouter();
  const storageKey = `legalsimple_divorce_with_children_chat_${caseId || 'draft'}`;
  ...
  const saveIntakeData = async (data?: any) => {
    ...
    try {
      const response = await fetch("/api/cases/save-intake", {
        ...
        body: JSON.stringify({
          caseId,
          intakeType: "divorce_with_children_chat",
          data: intakeData,
        }),
      });
      ...
      if (infoState.isComplete) {
        ...
        try {
          const response = await fetch("/api/cases/save-intake", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ caseId, intakeType: "divorce_with_children_chat", data: infoState.data }),
          });
