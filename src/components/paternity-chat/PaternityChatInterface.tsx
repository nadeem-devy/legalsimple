...
} from "@/lib/paternity-chat/engine";
import { getQuestionById, PATERNITY_QUESTIONS } from "@/lib/paternity-chat/questions";

interface PaternityChatInterfaceProps {
  caseId?: string;
...
function validateDateOfBirth(date: Date, questionId: string): { valid: boolean; error?: string } {
  ...
  // For petitioner/other party DOB - must be at least 18 years old
  const age = calculateAge(date);
  if (age < 18) {
    return { valid: false, error: "You must be at least 18 years old to file this petition." };
  }
  if (age > 120) {
...
export function PaternityChatInterface({
  caseId,
  onComplete,
}: PaternityChatInterfaceProps) {
  const router = useRouter();
  const storageKey = `legalsimple_paternity_chat_${caseId || 'draft'}`;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (currentQuestion.type === "info") {
      ...
      if (infoState.isComplete) {
        // Save inline (same logic as below)
        setIsSubmitting(true);
        setSaveFailed(false);
        try {
          const response = await fetch("/api/cases/save-intake", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ caseId, intakeType: "establish_paternity_chat", data: infoState.data }),
          });
    ...
    if (
      (currentQuestion.id === "full_name" || currentQuestion.id === "other_party_full_name" || currentQuestion.id === "child_name") &&
